//*Taken from an online sourse (see report for references)
#include <stdio.h>
#include <stdlib.h> //*
#include <time.h> //*
...
for (round_number = 1; round_number <= 3; round_number++)
{
  ...
  if (total_dice_roll = blackjack) { //Did the player reach 21? Yes, End round and add their round points to their total score.
    ...
    "You will now start the next round.", round_score, total_score); 
    ...
    printf("Your final total score was %d", total_score);
    ...
    printf("Your total number of points for that round was %d", round_score);
    printf("Your total score across all rounds is currently %d", total_score);
    break; 
    }
